...
export default function CourseDetailPage({
  ...
  useEffect(() => {
    async function fetchCourse() {
      try {
        let data = await getCourse(decodedSlug);
        if (!data) {
          // Try to find in COURSE_BOARDS
          for (const board of COURSE_BOARDS) {
            const foundCourse = board.courses.find(c => c.slug === decodedSlug);
            if (foundCourse) {
              data = {
                slug: foundCourse.slug,
                title: foundCourse.title,
                short: foundCourse.title,
                instructor: foundCourse.instructor,
                pricing: {
                  monthly: foundCourse.price as number,
                  annual: (foundCourse.price as number) * 12,
                },
                syllabus: foundCourse.subjects || [],
              } as CourseDoc;
              break;
            }
          }
