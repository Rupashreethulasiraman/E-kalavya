...
import { Atom, Award, FlaskRound, GraduationCap, HeartPulse, Sigma, ShieldCheck, Sparkles, Target, Users } from "lucide-react";
  ...
  return (
    <div className="min-h-screen" style={{ background: "#faf9ff" }}>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 grid md:grid-cols-2 gap-14 items-center">
        ...
        >
          <motion.span
            className="inline-flex items-center gap-2 text-sm font-bold tracking-widest uppercase px-4 py-2 rounded-full mb-6"
            style={{ background: "#ede9fe", color: "#6d28d9" }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.6 }}
          >
            <GraduationCap size={16} /> India's Most Loved Ed-Tech
          </motion.span>
          ...
          >
            {[
              { label: "10,000+ Students", icon: <Users size={16} /> },
              { label: "95% Success Rate", icon: <ShieldCheck size={16} /> },
              { label: "Expert Faculty", icon: <Award size={16} /> },
            ].map((badge) => (
              <span
                key={badge.label}
                ...
                style={{ color: "#6d28d9" }}
              >
                <span style={{ color: "#f59e0b" }}>{badge.icon}</span> {badge.label}
              </span>
            ))}
          </motion.div>
        </motion.div>
...
function TeacherCard({ teacher, idx }: {
  teacher: { name: string; degree: string; role: string; img: string; icon: React.ReactNode };
  idx: number;
}) {
  return (
    <motion.div
